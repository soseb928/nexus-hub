--[[
    NexusPlay Hub - GitHub UI Loader
    Loads the NexusPlay UI library from the owner's GitHub repository.
]]
local NEXUSPLAY_UI_URL = "https://raw.githubusercontent.com/soseb928/nexus-hub/refs/heads/main/NexusPlay_UI_Library.lua"

local function NexusPlay_LoadUI()
    local source = game:HttpGet(NEXUSPLAY_UI_URL, true)
    local fn = loadstring(source)
    assert(fn, "NexusPlay UI Library could not be compiled.")
    return fn()
end

local NexusPlayUI = NexusPlay_LoadUI()
...
function bossServerHop()
    ...
    pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "No boss here -- server hopping...", Type = "Info", Duration = 3 }) end)
    NexusAllowTp = true
    if best then
...
function NexusHRNotify(msg, kind)
    pcall(function()
        Library:Notify({ Title = "NEXUS HUB", Content = tostring(msg), Type = kind or "Info", Duration = 6 })
    end)
end
...
function NexusKMNotify(msg, kind)
    pcall(function()
        Library:Notify({ Title = "NEXUS HUB", Content = msg, Type = kind or "Info", Duration = 7 })
    end)
end
...
function NexusBannerSync()
    ...
    elseif labels[1] then
        ...
        pcall(function()
            Library:Notify({ Title = "NEXUS HUB", Content = "Banner rotation changed \226\134\146 now on " .. labels[1], Type = "Info", Duration = 5 })
        end)
    end
end
            ...
            pcall(function()
                ...
                hooked = hookfunction(original, function(...)
                    if AntiTpOn and not NexusAllowTp then
                        warn("[NEXUS HUB] blocked TeleportService:" .. name .. " (Anti Teleport ON)")
                        return nil
                    end
                    return hooked(...)
                end)
                    ...
                    if remain >= e[4] and have >= e[4] then
                        ...
                        if after > before then
                            ...
                            if fails >= 3 then
                                ...
                                fails = 0
                                pcall(function()
                                    Library:Notify({ Title = "NEXUS HUB", Content = "Auto Exchange stopped: the server rejected the exchange", Type = "Warning", Duration = 6 })
                                end)
                            end
                            task.wait(0.5)
                        end
...
NEXUS_LOADER_URL = "https://example.invalid/NexusPlayHub/loader"
AutoExecOn       = false
...
do
    ...
    local LIB_URL = "https://example.invalid/NexusPlayHub/loader"
    ...
    function Luna:CreateWindow(o)
        ...
        local ok, ui = pcall(function()
            return Luna.Lib:CreateWindow({
                welcome   = tostring(NEXUS_WELCOME or o.Name or "NEXUS HUB"),
    ...
    function Luna:Notification(o)
        ...
        o = o or {}
        local title = tostring(o.Title or "NEXUS HUB")
    ...
    if type(Luna) ~= "table" then
        warn("[NEXUS HUB] UI library did not return a usable object.")
        return
    end
...
CONNS[#CONNS + 1] = { Disconnect = function()
    ...
    function NexusSafeAct(plr, why, hop)
        ...
        NexusSafeBusy = true
        local msg = "[NEXUS HUB] Safety -- " .. why .. ": " .. plr.Name .. " (" .. tostring(plr.UserId) .. ")"
        pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = msg, Type = "Warning", Duration = 5 }) end)
    ...
    function NexusSafeAdd(txt)
        ...
        if txt == "" then
            pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Type a Username or User ID first", Type = "Info", Duration = 3 }) end)
            return
        end
        local Players = NEXUS_LV.Players
        local entry
        ...
        for _, e in ipairs(NexusSafeList) do
            if (entry.id and e.id == entry.id)
                ...
                pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = entry.name .. " is already on the list", Type = "Info", Duration = 3 }) end)
                return
            end
        end
        ...
        NexusSafeRefresh(true)
        pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Watching " .. tostring(entry.name), Type = "Success", Duration = 3 }) end)
        NexusSafeScanNow()
    end
    ...
    SettingsTab:CreateButton({ Name = "Check Server Now", Callback = function()
        ...
        if not NexusSafeOn() then
            pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Turn a Safety switch on first", Type = "Info", Duration = 3 }) end)
            return
        end
        NexusSafeRefresh(true)
        if not NexusSafeScanNow() then
            pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Safety: nobody in this server matched", Type = "Info", Duration = 3 }) end)
        end
    end })
    ...
    ;(function()
        ...
        local function nxSay(msg, kind)
            pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = tostring(msg), Type = kind or "Info", Duration = 4 }) end)
        end
    ...
    function NexusCopy(txt)
        ...
        pcall(function()
            if ok then
                Library:Notify({ Title = "NEXUS HUB", Content = "Copied: " .. txt, Type = "Success", Duration = 3 })
            else
                Library:Notify({ Title = "NEXUS HUB", Content = "Clipboard not supported by your executor", Type = "Error", Duration = 3 })
            end
        end)
    end
    ...
    StatusTab:CreateButton({ Name = "Copy Position", Callback = function()
        ...
        if not x then
            pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Character not loaded", Type = "Error", Duration = 3 }) end)
            return
        end
        NexusCopy(x .. ", " .. y .. ", " .. z)
    end })
    ...
    StatusTab:CreateButton({ Name = "Tp to Position", Callback = function()
        ...
        if not x then
            pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Type a position like 1347.7, 677.09, 410.25", Type = "Error", Duration = 3 }) end)
            return
        end
        task.spawn(function()
            local ok = false
            ...
            if ok then
                pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Teleported to " .. x .. ", " .. y .. ", " .. z, Type = "Success", Duration = 3 }) end)
            else
                pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Character not loaded", Type = "Error", Duration = 3 }) end)
            end
        end)
    ...
    StatusTab:CreateButton({ Name = "Tp to Part / Model", Callback = function()
        task.spawn(function()
            ...
            if name == "" then
                pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "Type a part or model name first", Type = "Error", Duration = 3 }) end)
                return
            end
            ...
            if total == 0 then
                pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "No part or model named \"" .. name .. "\" found", Type = "Error", Duration = 3 }) end)
                return
            end
            ...
            if not pos then
                NexusTpPartList = {}
                pcall(function() Library:Notify({ Title = "NEXUS HUB", Content = "That one despawned, press Tp again", Type = "Error", Duration = 3 }) end)
                return
            end
            local ok = false
    ...
    pcall(function()
        Library:Notify({ Title = "NEXUS HUB", Content = "Loaded successfully!", Type = "Success", Duration = 4 })
    end)
end
...
function NexusCapStaleFix(id)
    ...
    if pts > 0 then
        ...
        if not NexusStaleWarn[id] then
            ...
            pcall(function()
                Library:Notify({ Title = "NEXUS HUB", Type = "Error", Duration = 9,
    ...
    pcall(function()
        Library:Notify({ Title = "NEXUS HUB", Type = fixed and "Success" or "Info", Duration = 6,
            Content = fixed
