...
do
    ...
    -- ===== [NEXUSPLAY] Investigation Runtime Scanner =====
    local NEXUS_INV_SCAN_WORDS = {
        "investigation", "investigate", "edo", "nightmare",
        "essence", "objective", "hidden", "crate", "reward",
        "retry", "room", "sorcerer", "cursed",
    }

    local function NexusInvScanMatch(v)
        local t = string.lower(tostring(v or ""))
        t = string.gsub(t, "[^%w]+", "")
        for _, w in ipairs(NEXUS_INV_SCAN_WORDS) do
            if string.find(t, string.gsub(string.lower(w), "[^%w]+", ""), 1, true) then
                return true
            end
        end
        return false
    ...
    local function NexusInvScanPath(obj)
        local parts = {}
        local cur = obj
        local guard = 0
        while cur and guard < 40 do
            parts[#parts + 1] = tostring(cur.Name)
            cur = cur.Parent
            guard = guard + 1
        end
        local out = {}
    ...
    local function NexusInvScan()
        local hits, seen = {}, {}
        local function add(kind, obj, extra)
        ...
        pcall(function()
            ...
            if controllers then
                for _, d in ipairs(controllers:GetDescendants()) do
                    if NexusInvScanMatch(d.Name) then
                        add("CONTROLLER", d)
                    end
                end
            end
        ...
        pcall(function()
            local net = RepStorage and RepStorage:FindFirstChild("NetworkComm")
            if net then
                for _, d in ipairs(net:GetDescendants()) do
                    if (d:IsA("RemoteEvent") or d:IsA("RemoteFunction") or d:IsA("BindableEvent") or d:IsA("BindableFunction"))
                        and NexusInvScanMatch(d.Name) then
                        add("REMOTE", d)
                    end
                end
            end
        ...
        pcall(function()
            for _, d in ipairs(RepStorage:GetDescendants()) do
                if NexusInvScanMatch(d.Name) and (d:IsA("ModuleScript") or d:IsA("Folder") or d:IsA("Configuration")) then
                    add("REPLICATED", d)
                end
            end
        end)
            ...
            for _, d in ipairs(ws:GetDescendants()) do
                if NexusInvScanMatch(d.Name) then
                    ...
                    if d:IsA("Model") or d:IsA("BasePart") or d:IsA("Folder")
                        or d:IsA("ProximityPrompt") or d:IsA("RemoteEvent") or d:IsA("RemoteFunction")
                        or d:IsA("StringValue") or d:IsA("IntValue") or d:IsA("NumberValue")
                        or d:IsA("BoolValue") then
                        add("WORKSPACE", d)
                    end
                end
            end
        ...
        print("========== [NEXUS INVESTIGATION SCAN] ==========")
        print("Matches: " .. tostring(#hits))
        for i, h in ipairs(hits) do
            print(("[%03d] [%s] %s | Class: %s | Path: %s%s")
                :format(i, h.kind, h.name, h.class, h.path,
                    h.extra ~= "" and (" | " .. h.extra) or ""))
        end

        local controllerCount, remoteCount, workspaceCount, replicatedCount = 0, 0, 0, 0
        for _, h in ipairs(hits) do
            if h.kind == "CONTROLLER" then controllerCount = controllerCount + 1
            elseif h.kind == "REMOTE" then remoteCount = remoteCount + 1
            elseif h.kind == "WORKSPACE" then workspaceCount = workspaceCount + 1
            elseif h.kind == "REPLICATED" then replicatedCount = replicatedCount + 1 end
        end

        local summary = ("Controllers: %d | Remotes: %d | Workspace: %d | Replicated: %d | Total: %d")
            :format(controllerCount, remoteCount, workspaceCount, replicatedCount, #hits)

        pcall(function()
            Library:Notify({
                Title = "NEXUS INVESTIGATION",
                Content = summary .. ". Check executor console for the full scan.",
                Type = (#hits > 0) and "Success" or "Warning",
                Duration = 8,
            })
        end)
    ...
    NexusG.NexusInvestigationScan = NexusInvScan
    -- ===== [/NEXUSPLAY] Investigation Runtime Scanner =====
    ...
    pcall(function()
        ...
        InvestigationTab:CreateButton({
            Name = "Scan Investigation Runtime",
            Callback = function()
                task.spawn(function()
                    pcall(NexusInvScan)
                end)
        ...
        InvestigationTab:CreateParagraph({
            ...
            Content = "Run the scanner while an Investigation is open. It prints matching controllers, remotes, replicated modules, and workspace objects to the executor console so the automation can use the game's actual runtime API.",
        })
    end)
    -- ===== [/NEXUSPLAY] Investigation Automation =====
